  ...
  return (
    <View style={{position: 'relative', width: '100%', height: '100%', flex: 1}}>
      <MapView
          ...
          markers.map((marker, index) => {
            return (
              <Marker key={index} coordinate={marker.coordinate} title={marker.title}>
                <Image
                  source={
                    marker.type === EnumTypeMarker.doctor ? ASSETS.doctorPin : ASSETS.locationPin
                  }
        ...
        {!isSearch && userLocation.longitude !== 0 && userLocation.latitude !== 0 && (
      ...
      <Dialog
        ...
        <Text style={{color: '#000'}}>
          Recuerde que para hacer uso de este modulo tiene que activar su ubicación
        </Text>
        <Dialog.Actions>
          <Dialog.Button
